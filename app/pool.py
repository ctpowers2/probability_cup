...
from src.team_data import get_stats, CODE_TO_NAME
...
def build_matches() -> list[dict]:
    """The match slate with AI odds attached. One dict per match."""
    matches = []
    for mid, a, b, when in DEMO_FIXTURES:
        odds = ai_odds(a, b)
        favorite = max(OUTCOMES, key=lambda o: odds[o])
        matches.append({
            "id": mid,
            "team_a": a,
            "team_b": b,
            ...
            "ai_favorite": favorite,
            "ai_rationale": rationale(a, b, odds),
        })
    return matches


_MATCH_INDEX = {m["id"]: m for m in build_matches()}


def get_match(match_id: str) -> dict | None:
    return _MATCH_INDEX.get(match_id)
...
def record_pick(user: str, match_id: str, pick: str) -> None:
    ...
    if match_id not in _MATCH_INDEX:
...
def leaderboard() -> dict:
    ...
    if results:
        ai_briers, ai_correct = [], 0
        for mid, outcome in results.items():
            odds = ai_odds(_MATCH_INDEX[mid]["team_a"], _MATCH_INDEX[mid]["team_b"])
            ai_briers.append(_brier(odds, outcome))
            if max(OUTCOMES, key=lambda o: odds[o]) == outcome:
                ai_correct += 1
        rows.append({
            "name": "🤖 The AI", "is_ai": True, "picks": len(results),
            "avg_brier": round(sum(ai_briers) / len(ai_briers), 4),
            "correct": ai_correct, "total": len(results),
        })
